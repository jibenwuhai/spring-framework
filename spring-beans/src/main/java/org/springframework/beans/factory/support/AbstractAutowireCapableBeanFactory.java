...
public abstract class AbstractAutowireCapableBeanFactory extends AbstractBeanFactory
	...
	public Object applyBeanPostProcessorsBeforeInitialization(Object existingBean, String beanName)
		...
		//遍历容器为所有创建的bean添加所有的BeanPostProcess
		for (BeanPostProcessor processor : getBeanPostProcessors()) {
			//调用bean实例所有后置处理中初始化前的处理方法，为bean实例对象在初始化之前做一些自定义处理
	...
	protected Object createBean(String beanName, RootBeanDefinition mbd, @Nullable Object[] args)
		...
		try {
			...
			//如果bean配置了初始化前和初始化后的处理器，则视图返回一个需要创建bean的代理对象
	...
	protected Object doCreateBean(String beanName, RootBeanDefinition mbd, @Nullable Object[] args)
		...
		try {
			...
			//初始化bean对象，为bean实例对象应用BeanPostProcessor后置处理器
	...
	protected BeanWrapper createBeanInstance(String beanName, RootBeanDefinition mbd, @Nullable Object[] args) {
		...
		if (resolved) {
			if (autowireNecessary) {
				//配置了自动装配属性，使用容器的自动装配进行实例化。根据参数类型匹配bean的构造方法
	...
	protected Constructor<?>[] determineConstructorsFromBeanPostProcessors(@Nullable Class<?> beanClass, String beanName)
		...
		if (beanClass != null && hasInstantiationAwareBeanPostProcessors()) {
			...
			for (SmartInstantiationAwareBeanPostProcessor bp : getBeanPostProcessorCache().smartInstantiationAware) {
				//调用SmartInstantiationAwareBeanPostProcessor的determineCandidateConstructors方法，该方法可以返回要用于beanClass的候选构造函数
	...
	protected void populateBean(String beanName, RootBeanDefinition mbd, @Nullable BeanWrapper bw) {
		...
		//处理依赖注入，首选处理autowiring自动装配的依赖注入
		int resolvedAutowireMode = mbd.getResolvedAutowireMode();
		if (resolvedAutowireMode == AUTOWIRE_BY_NAME || resolvedAutowireMode == AUTOWIRE_BY_TYPE) {
			MutablePropertyValues newPvs = new MutablePropertyValues(pvs);
			// Add property values based on autowire by name if applicable.
			//将bean名称进行autowiring自动装配处理
	...
	protected void autowireByType(
		...
		//对Bean对象中非简单属性（不是简单继承对象，如8中原始类型）进行处理
		String[] propertyNames = unsatisfiedNonSimpleProperties(mbd, bw);
		for (String propertyName : propertyNames) {
			try {
				//获取指定名称属性描述器
				...
				//不对Object类型的属性精选autowiring自动依赖注入
				if (Object.class != pd.getPropertyType()) {
					//获取属性的赋值方法
					MethodParameter methodParam = BeanUtils.getWriteMethodParameter(pd);
					// Do not allow eager init for type matching in case of a prioritized post-processor.
					//检查指定类型是否可以被转换为目标对象类型
					boolean eager = !(bw.getWrappedInstance() instanceof PriorityOrdered);
					//创建一个要被注入的依赖描述
					DependencyDescriptor desc = new AutowireByTypeDependencyDescriptor(methodParam, eager);
					//根据容器的bean定义解析依赖关系。返回所要被注入的bean对象
	...
	protected void applyPropertyValues(String beanName, BeanDefinition mbd, BeanWrapper bw, PropertyValues pvs) {
		...
		for (PropertyValue pv : original) {
			...
			//属性值需要转换
				...
				//属性可转换,且属性原始值是字符串类型，属性元素类型值不是动态生成的字符串，属性的原始值不是集合或者数组类型的
	...
	protected Object initializeBean(String beanName, Object bean, @Nullable RootBeanDefinition mbd) {
		...
		try {
			//调用bean初始化方法，这个初始化方法是在spring bean定义配置文件中通过init-Method属性指定的
